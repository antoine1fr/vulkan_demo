...
void App::check_layers(std::vector<VkLayerProperties> available_layers_vec,
...
void App::create_vulkan_instance() {
...
void App::create_vulkan_surface() {
  SDL_bool result = SDL_Vulkan_CreateSurface(window_, instance_, &surface_);
  assert(result == SDL_TRUE);
}

void App::select_best_surface_format(VkSurfaceFormatKHR& surface_format) {
...
void App::create_vulkan_swapchain() {
  VkResult result;
...
std::vector<VkPhysicalDevice> App::enumerate_physical_devices(
    VkInstance instance) {
  VkResult result;
...
void App::enumerate_device_extensions(
...
void App::check_device_extensions(VkPhysicalDevice physical_device,
                                  std::vector<const char*> wanted_extensions) {
  std::map<std::string, VkExtensionProperties> available_extensions;
...
void App::create_vulkan_command_pool() {
...
void App::create_vulkan_command_buffer() {
...
void App::create_vulkan_pipeline() {
  VkResult result;
...
std::string App::load_file(const std::string& path, std::ios::openmode mode) {
...
VkShaderModule App::load_shader(const std::string& path) {
...
void App::end_frame(uint32_t image_index) {
...
void App::update_uniform_block(size_t frame_id,
...
std::vector<VkImage> App::get_swapchain_images() {
...
void App::create_vulkan_framebuffers() {
...
uint32_t App::find_memory_type(uint32_t type_filter,
...
void App::create_vulkan_buffer(VkBufferUsageFlags usage,
                               VkDeviceSize size,
                               VkBuffer* buffer,
...
void App::create_vulkan_vertex_buffer() {
...
void App::create_uniform_buffer_objects() {
...
void App::allocate_descriptor_sets() {
